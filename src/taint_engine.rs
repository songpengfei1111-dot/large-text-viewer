...
impl TracePath {
    ...
    pub fn render_as_tree(&self) {
        println!("\n=== 渲染追踪路径为二叉树 ===\n");
        let mut graph = Graph::new();
        Self::add_to_graph(self, &mut graph, 0);
        layout(&mut graph);
        render_to_stdout(&graph);
        println!("\n=== 二叉树渲染完成 ===\n");
    }

    fn add_to_graph(trace: &TracePath, graph: &mut Graph, next_id: usize) -> (usize, usize) {
        ...
        let line = format!("{}:{} {}{}", line_num, insn_name, insn_opt, mem_info);
        let display_text = if line.chars().count() <= MAX_LINE_LENGTH {
            line
        } else {
        ...
        let node_id = graph.add_node("", &display_text);
        let mut current_id = next_id + 1;
        
        for (idx, source) in trace.sources.iter().enumerate() {
            let (child_id, new_current_id) = Self::add_to_graph(source, graph, current_id);
            let color = if idx % 2 == 0 { EdgeColor::True } else { EdgeColor::False };
            graph.add_edge(node_id, child_id, color);
            ...
            .filter_map(|(idx, ((reg, _), pattern))| {
                ...
                self.service.find_prev(line_num, config)
                    .and_then(|prev| {
                        println!("    ✓ 找到 [行 {}]: {}", prev.line_number + 1,
                                 self.service.get_line_text(prev.line_number).unwrap_or_default());
                        self._trace_backward_node(prev.line_number, reg, depth + 1, tree)
                    })
                    .or_else(|| {
                        println!("    ✗ 未找到来源");
                        None
...
pub fn test_taint_new_arch() -> anyhow::Result<()> {
    use large_text_core::file_reader::FileReader;
    ...
    println!("\n=== 使用新架构追踪内存地址: ld__6cf01586a8_8 ===\n");
    if let Some(tree) = engine.trace_backward_tree(9217, "ld__6cf01586a8_8")? {
        println!("\n=== 追踪结果(新架构) ===\n");
        tree.print();
        println!("\n统计信息:");
